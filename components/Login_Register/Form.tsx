...
const Form = ({
  ...
  signIn: (arg1: FormData) => void;
  signUp: (arg1: FormData) => void;
  ...
  const isRegisterFormValid = () => {
    return (
      !isEmailInvalidRegister &&
      !isUsernameInvalidRegister.value &&
      !isPasswordInvalidRegister1.value &&
      !isPasswordInvalidRegister2.value
    );
  };
  ...
  const isRegisterFormFilled = () => {
    return (
      ...
      passwordInputRegister1.length > 0 &&
      passwordInputRegister2.length > 0 &&
      emailInputRegister.length > 0 &&
  ...
  const handlePasswordChange = () => {
    ...
    if (
      ...
      setIsPasswordInvalidRegister1({
        value: true,
        reason:
          "Le mot de passe doit contenir au moins 8 caractères, une majuscule, une minuscule, un chiffre, un caractère spécial et ne doit pas contenir d'espace",
      });
    } else {
  ...
  return (
    <>
      <div className="py-8 text-sm no-underline rounded-md text-foreground bg-btn-background hover:bg-btn-background-hover">
        <div onClick={() => router.back()} className="flex items-center cursor-pointer w-max">
          <svg
      ...
      <div className="flex flex-col items-center w-full h-full gap-4">
        <h1 className="mb-8 text-3xl font-bold sm:text-5xl text-textLight">SOCIAL QUEST</h1>

        <Tabs aria-label="Options" defaultSelectedKey="Se Connecter" onSelectionChange={handleSelectionChange}>
          <Tab key="Se Connecter" title="Se Connecter" className="w-full max-w-lg text-textLight correctOutlineNone">
            <form
              className="flex flex-col items-center justify-center w-full gap-6 animate-in text-foreground"
              action={() => handleLogin()}
            >
              <Input
                id="emailLogin"
              ...
              <div className="flex flex-col w-full gap-1">
                <Input
                  ...
                  endContent={
                    <button className="w-5 h-5 focus:outline-none" type="button" onClick={toggleVisibilityLogin}>
                      {isPasswordVisibleLogin ? (
                        <EyeSlashIcon className="text-2xl pointer-events-none text-default-400" />
                      ) : (
              ...
              {/* <Button variant='flat' onClick={loginWithGoogle2} className="flex items-center justify-start gap-2 p-2 font-bold text-black bg-white rounded-full">
                                <Image src={Google_logo} alt="Google logo" width={35} height={35} />
                                <span>Se connecter avec Google</span>
                            </Button> */}
              {searchParams?.message && (
                <p className="p-4 mt-4 text-center bg-foreground/10 text-foreground">{searchParams.message}</p>
              )}
            </form>
          </Tab>

          <Tab key="S'inscrire" title="S'inscrire" className="w-full max-w-2xl text-textLight">
            <form
              className="flex flex-col items-center justify-center w-full gap-6 animate-in text-foreground"
            ...
            >
              <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <Input
                  ...
                  endContent={<Spinner size="sm" className={`scale-75 ${!isTyping ? 'hidden' : ''} `} color="white" />}
              ...
              <Input
                ...
                endContent={
                  <button className="w-5 h-5 focus:outline-none" type="button" onClick={toggleVisibilityRegister1}>
                ...
                onChange={(e) => {
                  setPasswordInputRegister1(e.target.value);
                }}
              ...
              <Input
                ...
                endContent={
                  <button className="w-5 h-5 focus:outline-none" type="button" onClick={toggleVisibilityRegister2}>
                ...
                onChange={(e) => {
                  setPasswordInputRegister2(e.target.value);
                }}
                onBlur={handlePasswordChange}
              />
              <Button
                className="customButton bg-secondary/70 border-secondary text-2xl font-semibold !w-[90%]"
                type="submit"
              >
                S'inscrire
              </Button>
