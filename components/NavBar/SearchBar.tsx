  ...
  return (
    ...
    >
      {isLoading && (
        <AutocompleteSection
          title="Utilisateurs"
          classNames={{ group: 'flex flex-col gap-1', heading: headingClasses }}
          showDivider
        >
          {[1, 2].map((i) => (
            <AutocompleteItem
              value="Recherche en cours"
              textValue="Recherche en cours"
              key={`searchbar-recherche-en-cours-user-${i}`}
              className="animate-pulse h-12 bg-default-100 data-[hover=true]:bg-default-100"
            ></AutocompleteItem>
          ))}
        </AutocompleteSection>
      )}
        ...
        <AutocompleteSection title="Utilisateurs" classNames={{ heading: headingClasses }} showDivider>
          {profiles.map((profile: any) => (
            <AutocompleteItem
              ...
              onClick={() => {
                closeSearchBarMobile !== undefined ? closeSearchBarMobile() : null;
              }}
              key={`searchbar-${profile.username}`}
        ...
        >
          {[1, 2].map((i) => (
            <AutocompleteItem
              value="Recherche en cours"
              textValue="Recherche en cours"
              key={`searchbar-recherche-en-cours-guilde-${i}`}
              className="animate-pulse h-12 bg-default-100 data-[hover=true]:bg-default-100"
            ></AutocompleteItem>
          ))}
        </AutocompleteSection>
      ...
      {guildes && guildes.length > 0 && !isLoading && (
        <AutocompleteSection
          ...
          showDivider={inputValue ? true : false}
        >
          {guildes.map((guilde: any) => (
            <AutocompleteItem
              as={Link}
              href={`/g/${guilde.nom}`}
              key={`searchbar-${guilde.nom}`}
              value={guilde.nom}
              textValue={guilde.nom}
            >
              <div className="w-full h-full">
                <div className="flex flex-row w-full h-fit gap-2 items-center">
