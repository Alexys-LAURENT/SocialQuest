...
const PopoverUserProfile = ({ post }: { post: ExtendedPost }) => {
    return (
        <Popover classNames={{ content: "z-10", trigger: 'z-9' }} placement="top" offset={10} shouldBlockScroll={true} className='mr-3 md:w-[90vw] !max-w-[17.5rem] md:!max-w-sm min-w-[17.5rem]'>
            <PopoverTrigger>
            ...
            <PopoverContent className='p-0 bg-bgLightSecondary dark:bg-bgDarkSecondary'>
                <div className="flex flex-col w-full h-full">
                    <div className="w-full min-h-[4rem] md:min-h-[5rem] bg-white bg-cover bg-center transition-all rounded-t-md" style={{ backgroundImage: `url(${post.creator_banner_url})` }}></div>
                    <div className="relative w-full min-h-[1.5rem] md:min-h-[3rem]">
                        <div className="flex absolute -top-7 md:-top-10 left-5 gap-2 transition-all duration-500">
                            <Image src={post.creator_avatar_url || defaultUser.src} alt={post.creator_avatar_url || defaultUser.src} width={80} height={80} className="flex h-14 w-14 md:h-20 md:w-20 rounded-full text-large transition-all" />
                        </div>
                    </div>
                    <div className="flex flex-col p-4 gap-4">
                        ...
                        <div className="flex flex-col bg-[#2e2e2e] text-textLight gap-2 py-2 px-4 min-h-[7rem] max-h-[7rem] md:min-h-[10rem] md:max-h-[11rem] rounded-md">
                            ...
                            <div className="text-sm md:text-base overflow-y-auto">
                                {post.creator_a_propos === ""
                                    ? "L'utilisateur n'a pas encore rempli cette section"
                                    : post.creator_a_propos}
                            </div>
                        </div>
                        <Link id='redirectToUserProfil' href={`/${post.creator_username}`} className='hidden'></Link>
