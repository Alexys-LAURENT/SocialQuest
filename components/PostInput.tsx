...
const PostInput = ({ id_guilde, page, guildesUser, parent }: PostInputProps) => {
    ...
    async function send(e: any) {
        ...
        const isFilesDone = await uploadFile(imageData, 'images_posts');
        if (!isFilesDone) {
            ...
            <form id='NewPostinput' onSubmit={(e) => send(e)} >
                <div className="flex flex-col h-full w-full bg-bgLightCard dark:bg-bgDarkCard rounded-t-md py-2 px-6 gap-1 transition-all !duration-500">
                    ...
                    <div className="flex justify-center">
                        <div className={`flex flex-wrap gap-2 aspect-square ${imageData && imageData.length >= 1 ? 'w-full' : 'w-0'}`}>
                            {imageData && imageData.map((img, index) => (
                                <div className="relative flex-1-1 " key={index}>
                                    ...
                                    <button type='button' className="absolute top-1 right-1" onClick={() => setImageData(imageData.filter((_, i) => i !== index))}>
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 bg-[#000000]/40 rounded-full p-1 hover:bg-[#000000]/60 transition-all !duration-[125ms]" fill="none" viewBox="0 0 24 24" stroke="#fff">
