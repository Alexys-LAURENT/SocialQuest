...
const APropos = ({ isUserProfil, user, nextRewards, }: { isUserProfil: boolean; user: Profile | null; nextRewards: NextReward[] | null }) => {
  ...
  return (
    <div className="flex flex-col gap-6 h-[40%]">
      <div
        className={`flex flex-col gap-1 ${!isEditingAPropos ? 'h-[50%]' : 'h-full'} bg-tempBgLightSecondary dark:bg-tempBgDark border border-tempLightBorder dark:border-tempDarkBorder transition-all !duration-500 rounded-md py-2 px-3`}
      >
        <div className="flex font-semibold items-center justify-between">
          <h1 className="text-textDark dark:text-textLight transition-all !duration-[100ms]">A propos</h1>
          {isUserProfil &&
            (!isEditingAPropos ? (
              <PencilIcon
                className="w-4 h-4 ml-2 cursor-pointer"
                onClick={() => setIsEditingAPropos(!isEditingAPropos)}
              />
            ) : (
              <div className="flex">
                <CheckIcon
                  className="stroke-green-500 w-5 h-5 ml-2 cursor-pointer"
                  onClick={() => handleSaveEditAPropos()}
                />
                <XMarkIcon
                  className="stroke-red-500 w-5 h-5 ml-2 cursor-pointer"
                  onClick={() => handleCancelEditAPropos()}
                />
              </div>
            ))}
        </div>
        <div className="font-light text-sm h-full overflow-y-auto text-textDark dark:text-textLight transition-all !duration-[100ms]">
          {!isEditingAPropos ? (
            user?.a_propos !== '' ? (
              ...
              "Vous n'avez pas encore rempli cette section"
            ) : (
            ...
            <div className="h-full relative">
              <textarea
                ...
                maxLength={150}
                onChange={() => {
                  (document.getElementById('textAreaEditAProposLength') as HTMLSpanElement).innerText = (
                    document.getElementById('textAreaEditAPropos') as HTMLTextAreaElement
                  ).value.length.toString();
                }}
              />
              <div className="absolute flex gap-1 bottom-1 right-1 text-[10px] text-gray-300">
                <span id="textAreaEditAProposLength">{user?.a_propos?.length ?? 0}</span>/ 150
              </div>
            </div>
          )}
        </div>
      ...
      {!isEditingAPropos && (
        <div className="flex flex-col justify-center gap-2 h-[50%] bg-tempBgLightSecondary dark:bg-tempBgDark border border-tempLightBorder dark:border-tempDarkBorder transition-all !duration-500 rounded-md py-2 px-3">
          <div>
          ...
          <div className="flex justify-end items-center gap-1 text-end text-[#979797]">
            {isUserProfil && nextRewards && nextRewards.length > 0 && (
              <>
                <div className="text-sm text-[#777777] dark:text-[#919191]">
                  Suivant : {nextRewards.length > 1 ? `${nextRewards.length} items` : nextRewards[0].items.nom}
                </div>
                <div className="flex relative items-center justify-center rounded-md">
                  ...
                  >
                    <Image
                      className="!h-7 w-auto aspect-auto rounded-md"
                      src={nextRewards[0].items.image_url}
                      alt={nextRewards[0].items.nom}
                    ></Image>
                  </Image.PreviewGroup>
                </div>
              </>
