        ...
        <>
            {
                bannieres.map((banniere, index) => {
                    return (
                        <div key={`CardBannieresProfilePage${banniere.id_item_user}`} className="relative cursor-pointer h-full aspect-square bg-[#dadada] dark:bg-[#2e2e2e] rounded-md hover:opacity-70 transition-all" onClick={() => { isUserProfil && toggleItemSelected(banniere.id_item_user, banniere.is_equiped ? "desequip" : "equip"); router.refresh() }}>
