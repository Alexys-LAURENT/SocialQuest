...
const SelectedItem = ({ isUserInventory }: { isUserInventory: boolean }) => {
  ...
  return (
    selectedItem &&
    selectedItem.items &&
    selectedItem.items.id_item && (
      <div
        className={`min-w-[300px]  ${selectedItem ? 'flex w-full md:w-[300px]' : 'hidden md:flex'} flex-col bg-transparent`}
      >
        <ArrowLeftIcon
          onClick={() => setSelectedItem(null)}
          className="w-6 h-6 min-w-[24px] min-h-[24px] ms-4 mb-2 inline-block md:hidden cursor-pointer"
        />
        <DynamicSelectedItemContent
          selectedItem={selectedItem}
          setSelectedItem={setSelectedItem}
          isUserInventory={isUserInventory}
        />
      </div>
    )
  );
