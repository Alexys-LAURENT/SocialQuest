...
import { ModalItemInfoContext } from '@/app/context/ModalItemInfosContext';

const ShopItem = ({ item }: { item: itemShop }) => {
  const { openModalForItem } = useContext(ModalItemInfoContext);
  const { success, error } = useContext(ToasterContext);
  const router = useRouter();
  const handleBuyItem = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    e.preventDefault();
