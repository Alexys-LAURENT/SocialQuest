...
export async function checkSignUp(formData: FormData) {
    ...
    if (password.length < 8 || password.includes(' ') || !password.match(/[A-Z]/) || !password.match(/[a-z]/) || !password.match(/[0-9]/) || !password.match(/[^A-Za-z0-9]/)) {
        return redirect('/login?message=Le mot de passe doit contenir au moins 8 caractères, une majuscule, une minuscule, un chiffre, un caractère spécial et ne doit pas contenir d\'espace')
    }
